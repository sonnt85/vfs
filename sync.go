...
type Syncer struct {
	...
	SrcFs  afero.Fs //  afero.Fs SyncI
	DestFs afero.Fs // afero.Fs SyncI
}
...
// type  func( string) (os.FileInfo, error) lstat
...
func (s *Syncer) sync(dst, src string) {
	...
	if !sstat.IsDir() {
		...
		if !s.equal(dst, src, dstat, sstat) {
			...
			check(err)
			defer sf.Close()
			if s.SymbolicLink && sstat.Mode()&os.ModeSymlink != 0 {
				//  os.Lstat(name string)
				if dstLstat, ok := s.DestFs.(Lstat); ok {
					dstinfo, err := dstLstat.Lstat(dst)
					check(err)
					if dstinfo.Mode()&os.ModeSymlink == 0 {
						check(s.DestFs.Remove(dst))
					}
				} else {
					dstinfo, err := s.DestFs.Stat(dst)
					check(err)
					if dstinfo.Mode()&os.ModeSymlink == 0 {
						check(s.DestFs.Remove(dst))
					}
				}
				if rd, ok := s.SrcFs.(Readlink); ok {
					link, err := rd.Readlink(src)
					check(err)
					if sl, ok := s.SrcFs.(Symlink); ok {
						check(sl.Symlink(link, dst))
					}
				}
			} else {
				_, err = io.Copy(df, sf)
